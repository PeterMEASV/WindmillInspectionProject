...
class StreamCore {
    ...
    private attachGroupHandler(group: string) {
        ...
        const handler = (e: MessageEvent) => {
            console.log(`[StreamCore] RAW message received on group "${group}":`, e.data);
            ...
            // Use eventType from data if present, otherwise use the SSE event type (which is the group name)
            const eventType = data.eventType || e.type || group;

            console.log(`[StreamCore] Routing to listeners for eventType: "${eventType}" on group: "${group}"`);
            ...
            for (const listener of this.listeners.values()) {
                if (listener.group === group && listener.eventType === eventType) {
                    try {
                        ...
                        console.error(`[stream] Error in listener for "${eventType}":`, error);
                    }
                }
            }
        };
        ...
        console.log(`[stream] Attached handler for group: "${group}"`);
    }
